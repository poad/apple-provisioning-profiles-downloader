...
export class ProvisioningProfileDownloader {
    constructor(){
        this.run = async ()=>{
            try {
                const bundleId = core.getInput('bundle-id', JSON.parse("{\"required\":true,\"trimWhitespace\":true}"));
                const apiKeyId = core.getInput('api-key-id', JSON.parse("{\"required\":true,\"trimWhitespace\":true}"));
                const issuerId = core.getInput('issuer-id', JSON.parse("{\"required\":true,\"trimWhitespace\":true}"));
                const profileType = core.getInput('profile-type', JSON.parse("{\"required\":false,\"trimWhitespace\":true}"));
                const apiPrivateKey = core.getInput('api-private-key', JSON.parse("{\"required\":false,\"trimWhitespace\":true}"));
                const apiPrivateKeyFile = core.getInput('api-private-key-file', JSON.parse("{\"required\":false,\"trimWhitespace\":true}"));
                const tokenDuration = core.getInput('token-duration', JSON.parse("{\"required\":false,\"trimWhitespace\":true}"));
                if (apiPrivateKey.length === 0) {
                    ...
                    if (!apiPrivateKeyFileCheck) throw new Error("Specify either 'api-private-key' or 'api-private-key-file'.");
                }
                if (tokenDuration !== '' && Number.isNaN(tokenDuration)) throw new Error("The 'token-duration' must be an integer value.");
                const duration = tokenDuration !== '' ? Number(tokenDuration) : undefined;
                if (duration !== undefined && (duration < 1 || duration > 1200)) throw new Error("The 'token-duration' must be in the range of 1 to 1200.");
                if (!process.env.HOME) throw new Error('Environment variable `HOME` is not defined!');
                ...
                const profileIds = bundleIds.data.filter((value)=>value.attributes.identifier === bundleId && value.relationships.profiles !== undefined
                ).flatMap((bundle)=>bundle.relationships.profiles.data
                ).map((data)=>data.id
                );
                const profiles = bundleIds.included.filter((include)=>include.type === 'profiles' && profileIds.includes(include.id) && include.attributes.profileState === 'ACTIVE' && include.attributes.profileType === profileType
                ).map((include)=>include
                );
                if (profiles.findIndex((profile)=>profile.attributes.uuid !== undefined && profile.attributes.profileContent
