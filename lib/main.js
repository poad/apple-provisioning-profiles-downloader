...
const run = async ()=>{
    try {
        ...
        if (apiPrivateKey.length === 0) {
            const keyPath = _path.default.resolve(__dirname, apiPrivateKeyFile);
        ...
        const privateKey = apiPrivateKey || fs.readFileSync(_path.default.resolve(apiPrivateKeyFile));
        const client = _client.default.Client({
            privateKey,
            issuerId,
            apiKeyId,
            duration
        ...
        const basePath = _path.default.join(process.env.HOME, '/Library/MobileDevice/Provisioning Profiles');
        await io.mkdirP(basePath);
        profiles.map(async (profile)=>{
            const profileFilename = `${profile.attributes.uuid}.mobileprovision`;
            return {
                fullPath: _path.default.join(basePath, profileFilename),
